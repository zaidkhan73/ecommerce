...
import { ChevronLeft, Package, MapPin, CreditCard, Wallet, Truck, CheckCircle, Clock, XCircle, Star, ChevronDown, ChevronUp } from 'lucide-react';
import {useNavigate} from "react-router-dom"

function MyOrders() {
  const [orders, setOrders] = useState([]);
  const [expandedOrder, setExpandedOrder] = useState(null);
  const [ratingModal, setRatingModal] = useState({ show: false, orderId: null, productId: null });
  ...
  const submitRating = async () => {
    try {
      await axios.post(`${serverUrl}/api/orders/${ratingModal.orderId}/rate`, {
              ...
              >
                ...
                <div className="p-3 sm:p-4 lg:p-5 border-b border-gray-100">
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-3">
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      {/* Order Status */}
                      <span className={`flex items-center gap-1.5 px-2.5 sm:px-3 py-1 sm:py-1.5 rounded-lg text-xs sm:text-sm font-semibold border ${getStatusColor(order.order_status)}`}>
                        {getStatusIcon(order.order_status)}
                        {order.order_status.charAt(0).toUpperCase() + order.order_status.slice(1)}
                      </span>

                      {/* Payment Status */}
                      <span className={`px-2.5 sm:px-3 py-1 sm:py-1.5 rounded-lg text-xs sm:text-sm font-semibold border ${getPaymentStatusColor(order.payment_status)}`}>
                        {order.payment_status === 'paid' ? 'Paid' : '40% paid'}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="mt-3 pt-3 border-t border-gray-100 flex items-center justify-between">
                    <span className="text-xs sm:text-sm font-medium text-gray-600">Total Amount:</span>
                    <span className="text-lg sm:text-xl font-bold text-purple-700">₹{order.total_amount?.toFixed(0)}</span>
                  </div>
                ...
                {expandedOrder === order._id && (
                  <div className="p-3 sm:p-4 lg:p-5 space-y-3 sm:space-y-4 bg-gray-50">
                    {/* Order Items */}
                    <div>
                      <h4 className="text-sm sm:text-base font-semibold text-gray-900 mb-2 sm:mb-3">Order Items</h4>
                        ...
                        {order.items?.map((item, index) => (
                          ...
                          >
                            ...
                            {order.order_status === 'delivered' && (
                              <button
                                onClick={() => handleRateProduct(order._id, item.product_id)}
                    ...
                    <div className="bg-white rounded-lg p-3 sm:p-4 border border-gray-100">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <h4 className="text-sm sm:text-base font-semibold text-gray-900">Delivery Address</h4>
                      </div>
                      <p className="text-xs sm:text-sm text-gray-600 pl-6 sm:pl-7">{order.address}</p>
                    </div>
                    ...
                    <div className="bg-white rounded-lg p-3 sm:p-4 border border-gray-100">
                      <div className="flex items-center gap-2 mb-3">
                        ...
                        <h4 className="text-sm sm:text-base font-semibold text-gray-900">Payment Details</h4>
                      </div>
                      ...
                      <div className="space-y-2 pl-6 sm:pl-7">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs sm:text-sm font-medium text-gray-900">
                            {order.payment_method === 'online_payment' ? 'Full Online Payment' : 'Partial Payment (40% Online + 60% COD)'}
                          </span>
                        </div>
                        ...
                        {order.online_amount > 0 && (
                          <div className="flex justify-between items-center text-xs sm:text-sm">
                            ...
                            <span className="font-semibold text-gray-900">₹{order.online_amount?.toFixed(0)}</span>
                          </div>
                        )}
                        ...
                        {order.cod_amount > 0 && (
                          <div className="flex justify-between items-center text-xs sm:text-sm">
                            ...
                            <span className="font-semibold text-gray-900">₹{order.cod_amount?.toFixed(0)}</span>
                          </div>
                        )}

                        <div className="pt-2 border-t border-gray-200 flex justify-between items-center">
                          <span className="text-sm sm:text-base font-semibold text-gray-900">Total:</span>
                          <span className="text-base sm:text-lg font-bold text-purple-700">₹{order.total_amount?.toFixed(0)}</span>
                        </div>
                      </div>
                    </div>
                  </div>
      ...
      {ratingModal.show && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-2xl max-w-sm w-full p-5 sm:p-6">
            <h3 className="text-lg sm:text-xl font-bold text-gray-900 mb-4 text-center">Rate this Product</h3>
